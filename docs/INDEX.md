...
| `HANDOVER_METRICS_2026-08-08.md` | **短版交接，給 inpainting session**：為何不再用 `effect_siglip` 當主判定、改用的類別 margin 怎麼跑與怎麼校準、三層一起報、**抗淨化判定的現況與前提**、兩件會讓第二階段重蹈覆轍的事（影像選取的兩端、遮罩內讀出量） | **接手第二階段的判定部分時只需讀這份**。完整依據在 `METRICS` 與 `RESULTS` §7–§12 |
| `METRICS_2026-08-08.md` | **指標用法的常設參考**：74 個欄位分成三層（位移／語意／感知劣化）、每一層對齊哪幾篇論文、方向、以及各層的陷阱；`retention` 為何一直不可用；算不出來的指標與理由；引用時的三條規則 | **要引用任何指標之前先讀這份**。2026-08-08 實測到「同一批資料，三個判準給出三個相反的結論」，此後單一數字的引用都必須說明屬於哪一層 |
