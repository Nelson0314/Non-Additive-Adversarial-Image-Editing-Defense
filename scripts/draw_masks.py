...
量不到東西的原因。本工具把 c_a 顯示在標題列提醒。畫得好不好最後由
`src/defense/optimize.py` 反映出來：式 (5) 只計算 M 落在遮罩**外**的部分
（`restrict_outside_mask`，DEC-012），遮罩壓到主體愈多，能施力的區域就愈小，
小於 5% 即拋出。
...
def invert_masks(targets, out_dir: Path, guard: int = 13) -> List[tuple]:
    ...
    含有主體與補集的像素。故先把主體膨脹 `guard` 像素再取補集，兩者之間
    留一條帶，使遮罩不會緊貼輪廓。

    保護帶**不是**為了讓 M 與遮罩不相交——那條要求已經證實不可滿足
    （DEC-012）：M 是 c_a 的注意力圖，本來就會外溢到背景。它的作用只是
    讓主體邊緣不被劃進重畫區。
