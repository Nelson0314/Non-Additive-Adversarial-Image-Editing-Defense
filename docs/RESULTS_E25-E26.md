...
### 3.3 量測（真實 SD v1.4，512²，n_edit=10，n=2）

`scripts/p7_attack_sanity.py`。全部是**未防禦**的原圖，只差在 w 與 strength：

| 組態 | Δclip | Δsiglip | LPIPS→原圖 |
|---|---|---|---|
| **w=1.0, s=0.5（E2–E23 的設定）** | **+0.0116** | **+0.0351** | 0.4753 |
| w=3.0, s=0.5 | +0.0277 | +0.0640 | 0.4917 |
| **w=7.5, s=0.5（標準攻擊）** | **+0.0500** | **+0.0943** | 0.5232 |
| w=7.5, s=0.7 | +0.0563 | +0.0961 | 0.6215 |

標準 guidance 使 CLIP 對齊增益達 4.3 倍、SigLIP 2.7 倍。而現行設定的
Δclip = +0.0116 落在 §1.1 對照的噪聲範圍內（+0.0101 ± 0.0169）——**在 w = 1
下「編輯」與「什麼都沒做」在指標上分不出來**，與使用者的目視判讀一致。

單張 car_00 的現行設定甚至是 **Δclip = −0.0013**：編輯後對惡意 prompt 的
對齊度略為下降。
