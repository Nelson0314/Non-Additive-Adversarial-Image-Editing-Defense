...
`runs/` 只收**產出的影像**、**attention map** 與**殘差圖**。逐步的中間圖
**不要寫進 `runs/`**——不是寫出來再靠 `.gitignore` 擋，是一開始就不要寫。

| 留 | 不留 |
|---|---|
| `x_def*.png`、`edit_*.png`、`x_purified.png` | `x0_trace/`（逐步去噪的 x0 預測） |
| `*/attn/*_agg.png` | `history.png`（曲線圖，數值已在 `train.csv`） |
| `residual*.png` | 任何可由 CSV 重畫的圖 |
| `aligned_phi.png`、`baseline_phi0.png`（φ=0 參照，A 段驗收要用） | |

A 段新增的產物照同一條線：新的下限影像與差分圖要留，逐步最佳化的中間影像不留。
數值一律進 CSV／JSON。
...
- 一律**繁體中文**回答，客觀學術語氣；程式碼關鍵字、函式名、套件名維持英文。
  **commit message 用英文。**
- **中間圖不入庫**（DEC-017），只留產出的圖、attention map 與殘差圖
