"""批次報告頁。**純資料呈現，頁上沒有文字說明。**

條件由 CLI 給（`--tags`／`--conds`／`--short`／`--title`），所以同一支可以
產色散那批、也可以產影像引導那批。**缺的格子畫成占位而不是略過**——跑到
一半就出的報告要能一眼看出哪幾張還沒跑完。
...
# 由 CLI 填入（`--tags`／`--conds`／`--short`），模組載入時先給空值。
TAGS: list = []
COND: dict = {}
SHORT: dict = {}
...
def main() -> None:
    ...
    args = ap.parse_args()
    if not (len(args.tags) == len(args.conds) == len(args.short)):
        raise SystemExit("--tags／--conds／--short 的個數必須相同")
    TAGS[:] = args.tags
    COND.update(dict(zip(args.tags, args.conds)))
    SHORT.update(dict(zip(args.tags, args.short)))
    ...
    out = TEMPLATE.format(
        ...
        subtitle=html.escape(args.subtitle or
                             f"{len(names)} 張 · {len(TAGS)} 條件 · "
                             f"{len(purs)} 算子"))
