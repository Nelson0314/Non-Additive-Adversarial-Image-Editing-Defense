"""E0c — site L 的保真度下限 — spec §7.1 延伸、§7.2 T1 修訂紀錄的量化。

動機：site P 在 φ=0 時 `x_def = x` 逐元素相等，保真項為 0。site L 在
φ=0 時 `x_def` 仍是「VAE 編碼 → DDIM inversion(k_inv) → 去噪(k_inv) →
VAE 解碼」來回一趟的重建，本身即帶誤差，保真項有一個 φ 無法消除的下限。

這個下限直接影響 E2 的公平性：若不量出來，P 與 L 的防禦—保真前緣比較會把
「重建誤差」誤記為「防禦造成的失真」，兩個 site 的起跑點不同而圖上看不出來。

本腳本量測該下限隨 k_inv 的變化，並與 VAE 單獨來回（k_inv=0）分離，以區分
誤差來自 VAE 還是來自 DDIM inversion 的不精確。

判準：若下限的 PSNR 低於 LossConfig.psnr_floor（預設 30 dB），則該下限
