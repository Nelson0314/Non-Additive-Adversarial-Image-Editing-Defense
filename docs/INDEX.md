...
主線 25 張裡**只有 13 張的未防禦編輯真的執行了指令**，所有讀數只算那 13 張。
在未淨化的防禦上，紋理重相位要付 **4.36 倍**的失真才追得平 DCT-Shield；
在抗淨化上，它是**唯一一個防禦在淨化後還存在的方法**（最低存活率 50%，
三個 baseline 皆有算子歸零）。兩者必須並排讀，詳見
[RESULTS.md](RESULTS.md#目前的整體處境)。

判準已由位移改為**擋下率**（攻擊方拿不到可用輸出的比例），定義與代理指標見
