...
# 批次 profile
#
# `DESIGN` §2.0a（使用者 2026-08-06 定案）並行兩組實驗，兩組的
...
case "$BATCH" in
  ...
  ip*)
    ...
    #
    #       遮罩改為**人工繪製**（2026-08-09，DEC-010）：`--masks` 指到一個
    #       逐影像一張 PNG 的目錄，由 `scripts/draw_masks.py` 畫出來。文獻的
    #       作法即如此——PIE-Bench 附標註遮罩、PhotoGuard 與 AdvPaint 的
    #       inpainting 實驗用人工遮罩、Lo Figure 3 那張也是手畫的。
    #
    #       以下是**改用人工之前**的查證，全部留著，因為它正是不再用自動
    #       遮罩的理由：自動遮罩的涵蓋率與形狀是模型的性質，不是攻擊方的
    #       選擇，而那兩者都直接決定「攻擊方能改多少」。
    #
    #       ── 以下為 2026-08-08 的紀錄（`--mask-mode attention_box`）──
    #
    ...
    #       ── 紀錄結束 ──
    #
    #       上面那組涵蓋率是**自動遮罩**的性質，改人工之後不再是選圖的判準。
    #       人工繪製下的約束變成「主體之外還有沒有地方畫得下 0.15–0.45 的
