...
def test_逐像素取量值再池化等於沒有池化():
    ...
    P9 的實測顯示它與 `dchroma` 在每一個條件上數值完全相同。
...
def test_隨機色度雜訊在區塊內相消而連貫色偏不會():
    ...
    ΔE 那一類分不出來（P9 實測等 LPIPS 下雜訊 2.44 對色偏 2.79），
    而人眼分得出來。
    """
    ...
    # 兩個條件的振幅是量出來的，不是猜的：雜訊 0.06 與 R+0.12 使逐像素色度誤差
    ...
    assert 0.5 < dn / ds < 2.0, (
        f"兩個條件的逐像素色度誤差應相近才有比較意義，實得 {dn:.3f} vs {ds:.3f}")
