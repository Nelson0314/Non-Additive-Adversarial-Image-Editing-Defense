...
_t = DATA["paired"]["target"]
A("- :x: **與評測指標對齊的損失反而更差。**目標輸出直接最小化「離未防禦編輯的"
  "距離」，即評測所量的東西，其位移量卻低於注意力抑制 "
  f"{abs(_t['other'] / _t['attn'] - 1) * 100:.1f}%"
  f"（配對 n={_t['n']}，較優比例 {_t['win'] * 100:.0f}%）。"
  "訓練期的代理編輯鏈（10 步）與評測期的真實攻擊鏈（50 步）之間的落差，"
  "大於損失形式的差別。")
