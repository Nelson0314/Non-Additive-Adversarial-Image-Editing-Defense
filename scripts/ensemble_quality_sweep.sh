...
# 事前判準（跑之前寫下）
# ────────────────────────────────────────────────────────────────────
#   E1  **主判準。** jpeg30 的扣地板淨增益要高於現行 `ours_ph_q` 的 **0.1072**。
#   E2  jpeg75 不得掉超過 10%（現行是 0.3803）。集成若只是把優勢從高品質搬到
#       低品質，那是取捨不是改良，要照實寫成取捨。
#   E3  未淨化那一側（identity）不得低於現行 `ours_ph_q` 的 0.4657 的 90%。
#       過度特化到壓縮上而放掉未淨化的效果，與 `--deliver-jpeg` 已知的 −21%
#       是兩件事，不可混算。
#
