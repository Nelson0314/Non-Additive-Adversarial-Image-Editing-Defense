...
**未防禦編輯結果**與**防禦後編輯結果**對編輯 prompt 的對齊度。全專案沒有
...
LPIPS 距離很高但免疫完全失敗的案例。免疫成功應定義為**語意不匹配**（輸出
不再符合 prompt 的意圖）**或**感知劣化（輸出明顯是壞掉的）。本腳本量的是
前者，後者已由既有的失真指標涵蓋。
...
即**防禦抵銷了編輯效果的幾分之幾**。undo_frac = 1 表示編輯被完全擋下，
