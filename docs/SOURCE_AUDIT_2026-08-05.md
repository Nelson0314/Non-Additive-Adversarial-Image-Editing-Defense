...
| 3 | **CNN 去噪** | 冠軍模型不可得，**改用公開的強去噪器作替代**（Restormer／NAFNet／SCUNet 擇一），在報表標註「非 NTIRE 2023 冠軍，為我方替代」。**不得聲稱重現 DiffVax 的該項評測** | ✅ 需你確認替代對象 |
...
**建議取 (b)**：本專案的比較建立在匹配 τ_LPIPS 上，一個不受控的起點會讓
那個前提對 DIA-PT 失效；而加投影不改變其更新規則與損失，是所有 PGD 實作
的標準步驟。若取 (a) 或 (c) 請告知，改的是一行設定不是程式結構。
