...
# 這一行**不含任何反斜線**是刻意的：本腳本由別的語言產生再寫出，
# 反斜線跳脫會被多吃一層而寫出真正的控制字元留在原始碼裡，反倒讓腳本
# 自己的引號跨行。sed 的字元類別不需要跳脫，且只剔掉每行**行尾**的控制
# 字元，不會像 tr 那樣連換行一起吃掉。無條件跑，不先檢查。
sed 's/[[:cntrl:]]*$//' "$LIST" > "$LIST.tmp" && mv "$LIST.tmp" "$LIST"
N=$(wc -l < $LIST)
