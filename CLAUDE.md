...
- **A 族／生成路徑**：site L（latent ε）、site E（文字嵌入）、site W（權重 LoRA）。
  全部經 `decode(... encode(x) ...)`，共用同一個重建地板。BDIA 精確反演已實作
  並驗證（latent 來回誤差降 5 個數量級），消掉反演那一半後仍剩純 VAE 來回
  LPIPS 0.1434 / 27.51 dB。E17 顯示 latent 最佳化可把地板砍到 0.0760。
- **B 族／像素路徑**：site P（加性低秩）、site PF（加性全秩）、site S（空間變形）。
  無地板。

關鍵框定：那個地板是**我們選的 G** 造成的，不是威脅模型強加的。攻擊方用
