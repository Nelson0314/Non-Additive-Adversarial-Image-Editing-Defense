...
E22 的動機是「bicubic 條件被 `max_disp` 綁住」，但實測放寬四倍後位移與 LPIPS
...
已被 `scripts/e27_binding_check.py` 取代。本腳本只查 LPIPS 與鈍化
兩道 hinge，且 run 名稱寫死為 E15/E21/E22。後續發現的有效約束還包括硬上界、
防禦 margin 與係數為零的 hinge 被誤判，新腳本對任意 run 目錄運作並涵蓋全部
類別。保留本檔是因為 E22 的結論由它產生，重跑該結論仍需要它。
"""
