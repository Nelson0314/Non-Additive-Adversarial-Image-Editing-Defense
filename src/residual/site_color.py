...
(U, V)** 上；亮度 Y 原封不動地通過。輸出是原影像色度被**重新混合**的結果，
不是 `x + Δ`。
...
**Rec.601 亮度**的逐區塊梯度能量比（見該檔 `_grad_sq` → `_luma`）。本位置
...
特徵失真——色偏、色度串音、飽和區的假色——是否收費**尚未量測**。加入本位置
之後必須為它跑一次四臂等 LPIPS 探針，否則就是重蹈 site S 用 LPIPS 買模糊
的覆轍，只是換成用色度買。
...
**已知限制：近乎無彩的區域沒有容量。** M 作用在 (U, V) 上是**乘性**的，
...
**平滑度**由粗網格 + 雙線性上採樣的構造保證，與 site S 同一手法：由構造
保證比由懲罰項近似可靠，且 `grid_size` 就是一個乾淨的容量旋鈕。
"""
...
class ColorResidual(ResidualModule):
    ...
    def raw_residual(self) -> None:
        ...
        回傳 None 而非 `x_def − x`：後者是變換造成的**結果**，把它當成殘差
