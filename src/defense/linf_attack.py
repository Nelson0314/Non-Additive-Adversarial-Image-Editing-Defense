...
| 重建地板 | 有（純 VAE 來回即 LPIPS 0.1434） | **無** |

最後一列是關鍵。本專案 A 族（latent／embedding／weight）的防禦圖都要經過
`decode(... encode(x) ...)`，因此背負一個與防禦無關的重建地板；Lo 的方法是
純像素加性，`x_adv = x + δ`，沒有這個地板。把兩者塞進同一個函式會讓
「有沒有地板」這個結構差異藏在旗標裡。
