...
<p>表的位置寫成 <b>(row, col)</b>：</p>

<div class="tw"><table>
<tr><th></th><th>是什麼</th><th>數字愈大代表</th></tr>
<tr><td><b>row</b>（32 個）</td><td><b>垂直</b>方向一個區塊裡有幾個週期</td>
<td>上下方向的條紋愈密</td></tr>
<tr><td><b>col</b>（17 個）</td><td><b>水平</b>方向一個區塊裡有幾個週期</td>
<td>左右方向的條紋愈密</td></tr>
</table></div>

<p>兩個數字合起來就決定一個斜條紋圖案：<b>方向</b>是
<code>atan2(row, col)</code>，<b>粗細</b>是 32 / √(row² + col²) 像素。</p>

<div class="tw"><table>
<tr><th class="n">(row, col)</th><th>是什麼樣子</th></tr>
<tr><td class="n">(0, 0)</td><td>完全平坦，就是這一塊的平均亮度（DC）</td></tr>
<tr><td class="n">(0, 1)</td><td>整塊裡只有一個週期的左右漸層，週期 32 px</td></tr>
<tr><td class="n">(0, 4)</td><td>四道直條紋，週期 8 px</td></tr>
<tr><td class="n">(4, 0)</td><td>四道橫條紋，週期 8 px</td></tr>
<tr><td class="n">(4, 4)</td><td>45° 斜條紋，週期 5.7 px</td></tr>
<tr><td class="n">(28, 4)</td><td>−45° 斜條紋，週期 5.7 px（row 28 就是 row −4）</td></tr>
<tr><td class="n">(0, 16) 或 (16, 0)</td><td>最細的條紋，週期 2 px，一黑一白</td></tr>
</table></div>

{img("f7e_basis.png", "圖 7　把表上幾個位置畫成像素。左上角那格是平的（區塊平均），愈往外條紋愈密，row 與 col 的比例決定傾斜角度。")}

<h4>為什麼 row 有 32 個而 col 只有 17 個</h4>

<p>row 走完一整圈：<b>0 到 16 是往下的方向，17 到 31 其實是負的</b>——row 28 等於
row −4，與 row 4 同樣粗細但傾斜方向相反。</p>

<p>col 只存 0 到 16。影像是實數，它的頻譜有一半是另一半的鏡像
（(−row, −col) 那格是 (row, col) 的共軛），存兩份是浪費，<code>rfft2</code>
因此只存 col ≥ 0 這半邊。</p>

{img("f7f_index_map.png", "圖 8　表上每一格離中心多少個週期。左邊是實際存放的順序（row 0 在最上，往下到 16 之後折回負的）；右邊把 row 移到中間，就看得出來它其實是以 0 為中心對稱的。")}

<div class="note">
<b>這就是 4.3 那兩行要特別處理的原因。</b>col = 0 與 col = 16 這兩行的鏡像
（−col）剛好等於自己，所以它們的鏡像不在「沒存的那一半」，而是<b>在自己這一半之內</b>。
它們因此必須自己對 row 共軛對稱，逐格亂轉會破壞這個關係。
</div>
...
{img("f7_radial_gate.png", "圖 9　左：表上每一格離中心多遠。中：主線的閘，黑色是不能轉的格。右：最左與最右兩整行為什麼也要凍。")}
...
{img("f7d_radial_steps.png", "圖 10　閘的三個步驟。左：每格離中心多遠，是連續值。中：套上半徑門檻，只有中心一小圈變黑。右：再把最左最右兩行清掉——黑色面積主要來自這一步。")}

{img("f7b_rmin_compare.png", "圖 11　三個 r_min。差的格數不多，但差的都是最粗的那幾圈，那正是失真最貴的地方。FND-042 實測：r_min = 0.40 拿到主線 87% 的效果，只付 26% 的 DISTS 與 74% 的 LPIPS，PSNR 還高 5.9 dB。主線的 0.12 是照「可達的 DISTS 天花板最高」選的，不是效率上的操作點。")}
...
{img("f8_texture_gate.png", "圖 12　兩關的分數與相乘的結果。coherence 高的地方是邊緣，梯度能量低的地方是平坦區，兩者都被壓掉。留下來的黃色是草叢與皮毛。")}
...
{img("f8b_gate_examples.png", "圖 13　三個區塊在原圖上的位置與各自的 32×32 內容。紅框是鬍鬚，紋路全部同一方向；藍框是虛化背景，幾乎沒有紋路；綠框是草，方向雜亂而且夠強。")}
...
{img("f7c_brightness.png", "圖 14　左：轉 DC 時亮度隨 θ 走 cos 曲線（紅），與 cos θ 逐點重合；凍住 DC 時是一條平線（綠）。右：兩層凍結的說明。")}
...
{img("f9_theta_sweep.png", "圖 15　raccoon_00 上的 θ 掃描（此處 θ 是隨機初始化後全部拉到同一絕對值，只為展示幅度效果）。上排是輸出，下排是殘差放大 8 倍。殘差不是均勻的噪聲——它精確地落在草叢與皮毛上，那正是紋理閘留下的區域。")}

{img("f9b_crop.png", "圖 16　同一批的 96×96 放大。即使 θ = π，場景、物件位置、邊緣全部維持；改變的是草與毛的紋素排列。這與圖 1 下排的全域 RPN 形成對比——同樣是只轉相位，切塊加閘之後內容完全保住。")}

{img("f10_theta_curves.png", "圖 17　θ 與三個量的關係。PSNR 單調下降，L∞ 單調上升，amp_dev 在 θ≈2.2 之後反而回落——超過該點之後相位旋轉開始互相抵消。")}
...
{img("f11_real_pipeline.png", "圖 18　cat_00 的完整一列。上排：原圖與三個條件的防禦圖，肉眼幾乎分不出。中排：殘差放大 10 倍——加性的幾乎看不見（它住在高頻），相位的則貼著紋理分布。下排：SDEdit 編輯的結果。未防禦的編輯把貓變成狗；加性防禦下編輯幾乎照常成功；紋理重相位下編輯被推得最遠。")}

{img("f12_residual_spectrum.png", "圖 19　同一張圖的殘差徑向功率譜。加性擾動的能量集中在高頻末端，相位擾動的分布明顯較寬。這是「加性把能量放在 VAE 敏感、人眼不敏感的高頻」這個既有觀察（FND-026）的直接呈現。")}
...
<tr><td>能量落在哪</td><td>高頻（圖 19）</td><td>紋理區，跟著影像內容走（圖 18 中排）</td></tr>
...
徑向頻率閘（區塊內的低頻凍結）、紋理閘（邊緣與平坦區凍結）。圖 1 與圖 16 的對比
就是這個限制的效果。</p></details>
