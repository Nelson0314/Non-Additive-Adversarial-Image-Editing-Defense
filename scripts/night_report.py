...
還原。</p>
<h3>8.3 DiffusionGuard：補上「為抗淨化而設計」這一類</h3>
<p>前三項回答的是「會不會只是……」，這一項補的是另一個缺口：<b>現有的三個強
baseline 沒有一個是為抗淨化設計的</b>，而主軸已經改成抗淨化。</p>
<p>它的新意在<b>損失</b>——在<b>最高噪聲的那一個時間步</b>上，最大化 UNet 噪聲
預測的 L2 範數，每次迭代重抽噪聲。擾動本身就是普通的像素加性。所以本專案把
損失獨立出來，可以配上任何參數化，<b>包括相位臂</b>。</p>
<p class="note"><b>這是移植不是重現，已標註。</b>原文的威脅模型是 inpainting，
9 通道 UNet 讓影像與噪聲各佔一組通道；img2img 下影像只能經 SDEdit 的起始
latent 以 <code>√ᾱ<sub>t₀</sub></code> 加權混入（strength 0.7 時是 0.287），
梯度因此要穿過那個係數。而 mask-augmentation——原文兩個主要貢獻之一——在
img2img 下沒有對應物，完全拿掉。<code>modified_from_paper</code> 被強制為真。</p>
<h3>8.4 把可微分 JPEG 放進最佳化迴圈</h3>
...
{table(["檔案", "內容"],
       [["<code>src/residual/spectral_split.py</code>",
        ...
        ["<code>src/baselines/diffusionguard.py</code>",
         "DiffusionGuard 的早期時間步損失，移植到 img2img"],
...
（新增 175 項）。<code>CLAUDE.md</code> 記載的基準 196 早已過時。</p>
<div class="key">
...
表格照樣填滿、比值照樣有大有小。現在它們分別對<b>解析上已知的輸入</b>
（單一餘弦只佔一個箱、棋盤格落在角落）、<b>搜尋的收斂性</b>、以及
<b>合成 CSV 的已知答案</b>釘住。三段全部通過，<b>本報告的數字不受影響</b>。</p>
</div>
...
<code>Connection timed out</code>，四輪重試共三十餘次全部失敗），結果無法取回。
批次以 <code>setsid nohup</code> 啟動，不受連線中斷影響，資料應已存在
