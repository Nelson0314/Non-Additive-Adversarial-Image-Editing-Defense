...
第 1 族仍是文獻的主流（2026 年的 TPAMI 論文用它），本專案已兩次獨立量到它
與編輯是否失敗不對應。但「不對應」至今是**質性的敘述**——726 格語意失敗 0 格
說的是第 2 族沒有正例，不是兩族之間沒有關係。

本腳本直接量三族之間的等級相關（Spearman ρ）。SURVEY §8 把
「判準的三族之間如何換算」列為沒有人回答過的問題，而本專案的
`runs/*/results.csv` 有現成的材料——三族的欄位自 E2 起就一起記在同一列。

## 讀法

- ρ 接近 0：兩族量的是無關的東西。第 1 族的高分不預測第 2、3 族的高分。
- ρ 顯著為負：第 1 族越高、第 2 族越高（即防禦圖被編輯後**更**服從 prompt），
...
def spearman(xs, ys):
    """等級相關。用等級而非原值：三族的尺度與分佈都不同，Pearson 會被
...
def collect(runs_root: Path):
    ...
    只取無淨化那一列（`identity`、strength 0）：淨化臂的 `net_lpips` 已扣掉
    對照，但三族的比較在乾淨攻擊上最乾淨，混入淨化會多一個變因。
    """
...
def main():
    ...
    print("[p16] 讀法見本檔 docstring。ρ 接近 0 代表兩族量的是無關的東西——"
