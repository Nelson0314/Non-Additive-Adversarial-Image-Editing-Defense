...
為什麼需要一個常設工具。匹配失真的比較只有在兩個條件都被同一道約束
綁住、且都已在該約束下停下來時才成立。本專案已經連續踩到四個不同的有效約束，
...
# 於是 `gamma_psnr=0`（保留計算與記錄但不參與梯度）的 PSNR 罰則被當成有效約束
...
def analyse(run_dir: Path) -> list:
    ...
    for hf in sorted(run_dir.glob("*/history.json")):
        ...
        if clamp and clamp[1] > 0.9:
            ...
            # 只有一道綁住是不同的狀態：跨 site 比較要求兩個條件被同一組約束
            # 綁住，若一條件是 LPIPS、另一條件是 LPIPS＋色度，該對就不可並列。
