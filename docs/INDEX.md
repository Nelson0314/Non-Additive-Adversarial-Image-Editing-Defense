...
> 每個 run 目錄屬於哪個實驗。
>
> **接手實驗請先讀 `HANDOVER_2026-08-07.md`**——它自足，讀完就能接手；
> 需要細節時再依其 §9 的地圖跳到本表的對應文件。
>
...
| `RESULTS_2026-08-07.md` | **兩組段 0 通過、段 1–3 執行中**：段 0 三項檢查的數據、本日四項缺陷、**訓練實際被 `tau_acut` 而非 `tau_lpips` 綁住的量測**（§4）、τ=0.20 上兩道 hinge 的逐條件實測與門檻建議（§5）、對齊指標的限制與四個替代方案（§6） | **要引用 08-07 的數字時讀這份**；兩項待裁決事項的資料都在這裡 |
| `HANDOVER_2026-08-07.md` | **現行交接**：機器與連線、正在跑的工作與重啟監控的指引、兩組實驗的設定、2026-08-06 的九個缺陷清單、待裁決事項、例行操作 | **接手時先讀這份** |
...
| `b3` | SDXL、1024²、bf16 | **SDXL 正式那一組**。段 0 執行中。帶 2026-08-06 的全部修正，b1／b2 的格點因 `TRAIN_TAU` 由 0.35 改為 0.20 而全部失效 |
| `v14` | SD v1.4、512²、fp32 | **第二組實驗**。段 0 執行中。依 `DESIGN_2026-08-05` §2.0a：Mist 與 DIA 在此為原生設定，且像素數 1/4 |
