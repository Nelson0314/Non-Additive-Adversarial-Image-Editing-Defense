...
主線影像是十三張（`runs/ip2p_fair_comparison/images13.txt`）——25 張的人眼
稽核顯示只有這十三張的未防禦編輯真的執行了指令，其餘沒有攻擊可防。而**指令
的型態決定服從率**，擴充樣本時一律取「針對畫面主體」的指令。
...
`HANDOFF.md` 是**貼給新 session 的第一則訊息**，不是常設文件——它有時間相依
性（現況、在跑什麼、下一步），換一輪就整份重寫。常設的判準仍以
`DECISIONS.md` 與 `RESULTS.md` 為準。
