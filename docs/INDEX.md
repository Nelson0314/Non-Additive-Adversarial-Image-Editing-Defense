...
| `RESULTS_2026-08-06.md` | **段 1 首次在 GPU 上執行的實測紀錄**：六項缺陷（三個崩潰、三個使結果無效）、其根因與修法、生成路徑重建下限的完整量測、SD v1.4/512² 的對照量測、四項待裁決事項、逐格機時 | **要引用今天的任何數字時讀這份**；`RUNBOOK` 的旗標為何是那樣也在這裡 |
| `HANDOVER_2026-08-06.md` | **上機實況交接**：段 0 完成後的現狀、當天修掉的 7 個缺陷與依實測改掉的 3 個參數、操作指南、待辦與已知風險 | **接手正在跑的批次時先讀這份，再讀 RUNBOOK** |
...
| `b2_<影像>` | 同上 + `--exact-inversion --t-max 200 --purify-mode all` | 只跑 N3，驗證反演修正。三張圖 |
...
| `v14` | SD v1.4、512²、fp32 | **第二組實驗**。依 `DESIGN_2026-08-05` §2.0a：Mist 與 DIA 在此為原生設定，且像素數 1/4 |
