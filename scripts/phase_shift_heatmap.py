...
def main() -> None:
    ...
    floor = args.src / "purify" / "gallery_floor"
    rows = []
    for img in args.images:
        ...
        # 守門：`identity` 下 `δ' ≡ δ`，相位恆為 0、能量存活恆為 1。
        ...
        for op in OPS:
            ...
            maps.append((OP_LABEL[op],
                         residual_maps(delta, load01(pd) - load01(po))))
        rows.append((img, maps))

    ncol = len(OPS)
    nrow = len(rows)
    fig, axes = plt.subplots(nrow + 1, ncol,
                             figsize=(2.25 * ncol, 2.7 * nrow + 3.8),
                             gridspec_kw={"height_ratios": [1] * nrow + [1.0]})
    axes = np.atleast_2d(axes)
    im = None
    for r, (img, maps) in enumerate(rows):
        for c, (title, (ph, _)) in enumerate(maps):
            im = panel(axes[r][c], ph, title if r == 0 else "")
        axes[r][0].set_ylabel(img.replace("task_attr_mod_color_", "影像 "),
                              fontsize=8.5)

    # 最後一列：兩個量的徑向剖面。實線是相位轉動（左軸），虛線是能量存活
    # （右軸）。必須並排讀——模糊是「相位低、能量低」，JPEG 是「能量高、
    # 相位高」，兩者壞掉的方式不同。
    for c in range(ncol):
        ax = axes[nrow][c]
        ax2 = ax.twinx()
        for r, (img, maps) in enumerate(rows):
            ph, sv = maps[c][1]
            lab = img.replace("task_attr_mod_color_", "影像 ")
            rr, pp = radial_profile(ph)
            ax.plot(rr, pp, lw=1.5, color=f"C{r}", label=lab)
            rr2, ss = radial_profile(sv)
            ax2.plot(rr2, ss, lw=1.2, color=f"C{r}", ls="--")
        ...
        if c == 0:
            ax.set_ylabel("實線：|Δφ|（弧度）", fontsize=8)
            ax.legend(fontsize=7, frameon=False, loc="upper left")
        else:
            ax.set_yticklabels([])
        if c == ncol - 1:
            ax2.set_ylabel("虛線：能量存活率", fontsize=8)
        else:
            ax2.set_yticklabels([])

    fig.suptitle(
        f"淨化把「擾動的相位」轉了多少　條件 {args.cond}\n"
        f"δ = 防禦圖 - 原圖,  δ' = T(防禦圖) - T(原圖)；"
        f"熱圖 = Σ_b |R|·|Δφ| / Σ_b |R|，直流在中心、外圈 Nyquist\n"
        f"色階上限固定取 π/2 = {NO_INFO:.3f}（無資訊水平）："
        f"亮＝與噪聲無異，暗＝相位真的被保住了",
        fontsize=10.5)
    cbar = fig.colorbar(im, ax=axes, fraction=0.014, pad=0.035)
    cbar.set_label("|Δφ|（弧度），上限 π/2", fontsize=8)

    fig.text(0.01, 0.012,
             "高斯模糊在頻域乘的是實正數 exp(-2π²σ²f²)，構造上不可能改相位"
             "——它抽掉的是載體，所以應該是「實線低、虛線低」。"
             "JPEG 量化把係數推過整整一階，能量比 1 還大而方向被打散，"
             "應該是「虛線高、實線高」。\n"
             "裁切縮放是繞中心的純放大 1.2488×，區塊格點與原圖對不上，"
             "該欄量到的是對位破壞不是相位破壞，不可與其餘欄並列解讀。"
             "π/2 = 1.571 是兩個獨立均勻角度之差的期望。",
