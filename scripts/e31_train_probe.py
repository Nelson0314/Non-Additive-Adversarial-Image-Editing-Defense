...
E29 在 H100 上實測 10.3 GB。本機只有 4 GB，512² 的訓練無法執行是已知的
...
**本腳本印出的 `shift` 不是任何東西的量測**（2026-08-04 補記，LEDGER 5.14）。
它只傳 `[Purifier("identity")]`，而 `untargeted` 的防禦項在兩條分支逐元素
相同時梯度精確為零（5.10），故 φ 每一步都停在 0，`shift` 恆為 0。
成本數字仍然有效——前向與反傳照跑，只是參數沒有更新。
