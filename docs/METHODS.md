...
- **為什麼必須跑對齊**：不對齊的話它的 `x_base` 是未對齊的 VAE 重建，
  同一個 τ 之下得先付掉更大的重建誤差，留給隨機方向的預算比 apa 少
...
條件本身不變：apa、Ra 加三個 baseline，位移場三個不放回來（DEC-005）。
...
## MTH-apa_rd · site apa + 注意力抑制取比例（`redirect_attn_ca`）

- **參數化**：與 MTH-N4 相同
- **損失**：`‖Att(x_def, c_a) ⊙ M‖₁ / ‖Att‖₁`，值域 [0, 1]
- **與 MTH-N4 的差別只有分母**，但那決定可達成的手段：分子式可以靠把整張
  注意力圖一起壓低來降低（而那是去噪鏈後續步數會自行補回的擾動）；取比例後
  分子分母同時縮小，只有把質量真的移到 M 外才會下降
- **學習率鍵自己一組**（`lr.N5_stage2`）：比例落在 [0,1] 而 L1 量級 100–300，
  `_pick_best` 取末端總損失的 argmin，同一個值沒有理由同時適用；共用還會讓
  `calibrate_lr` 在同批跑兩者時靜默覆寫
- **結果**：FND-024 第 4 條

## MTH-apa_pj · site apa + 注意力抑制 + 投影式約束

- **損失**：與 MTH-N4 逐字相同
- **唯一差別**：保真約束由 hinge 改為每步投影（DEC-019）
- **附帶要求**：必須用隨機起點。方向參數依 LoRA 慣例初始化為零，而零向量
  沒有徑向投影（DEF-017）。起點取高斯、種子由影像 id 決定，與 MTH-Ra 同一
  條規則，故兩者的起點分布相同
- **結果**：`scale_k` 0.938–1.000、`|1−銳利度比|` 0.2235 → 0.0623

## MTH-apa_tj · site apa + targeted_output + 投影式約束

- **損失**：`‖SDEdit_sub(x_def; c_∅) − y_target‖²`，`y_target` 為
  `data/targets/gray.png`（512² 全值 128 的純灰圖）。訓練期一律餵空 prompt
- **設計動機**：投影已證明有效（MTH-apa_pj），而注意力著力點已證明無效
  （FND-024）。本條件把有效的約束套到**與評測量同一件事**的損失上，是兩者
  各自成立後唯一還沒試過的組合
- **成本**：每步需完整的可微編輯鏈（10 步 UNet 帶梯度）加投影，實測
  893–1591 秒／格，為注意力路線的 4–7 倍
- **結果**：位移量 0.0776，四個 arm 中最差。見 EXP-s3t20_tj
