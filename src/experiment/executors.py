...
# `write_csv` 與 `load_image_tensor` 已移到 `src/utils/io.py`（PLAN.md §6.1a）。
# 此處重新匯出，使舊主線既有的 `executors.write_csv` 呼叫維持不變——複製一份
# 實作會讓兩邊的 CSV 欄位規則慢慢分岔，而既有 runs/ 會變成不可比。
from src.utils.io import load_image_tensor, write_csv   # noqa: E402,F401
